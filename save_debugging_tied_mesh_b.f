...
c
     ...
     &  numelemface,numnodeperface,numcornerperface, nowcase,
     ...
     &           tied_node_is_a_master_node, hex_elem, tet_elem, 
     &           wedge_elem, l1, l2, l3, l4, l5, l6, l7, l8
c
      double precision ::  
...
c
      do nd = 1, numtiednode    !  *** loop over all slave nodes ****
        ...
        node_per_elem = iword
        call set_element_type( elemchoice, l1, hex_elem, wedge_elem,
     &                         tet_elem, l2, l3, l4, l5, l6, l7, l8 )
c
...
c
        nowcase = 0
        if( hex_elem ) nowcase = 1
        if( tet_elem ) nowcase = 2
c        
        select case( nowcase )
        case(1)      ! hex element
          isopoint = zero
...
c
c               is the slave node actually a node of the
c               master element? if so, we skip the iterative
c               procedure to locate slave node position on the
c               master element face.
c
        call tied_nodeiso_ee
...
c
      integer :: ii, jj, etype, idummy, enode
      double precision :: enode_coords(3), ddummy, distance,
     &                    xi, eta, zeta
      logical :: matched
c        
...
c
      do ii = 1, numnodeperface
       ...
       distance = srt_dist3d( slavenode, enode_coords, 3 ) 
       matched = distance <=  epsilon*character_size 
       if( .not. matched ) cycle 
       tied_node_is_a_master_node = .true.
       etype = elemchoice  ! protect elemchoice value in case
       if( hex_elem ) 
     &    call ndpts1( idummy, numnodeperface, ddummy, etype, enode,                   
     &                 isopoint(1), isopoint(2), isopoint(3) )
       if( tet_elem ) then
          jj = ii
          call srt_tet_fcoor( face_id, numnodeperface, etype, enode,                   
     &                 jj, xi, eta, zeta )
         write(*,*) '...   slave node match master element'
          write(*,*) '    slave node, master element:',node_id,elem_id
         write(*,*)  '    face_id ',face_id
         write(*,*)  'ii, enode, etype: ', ii, enode, etype
...
c
      ...
      subroutine srt_tet_fcoor( face_id, nfnode, etype, enode,
     &                          fnode_pos, xi, eta, zeta )
c
      integer :: nfnode, etype, enode, face_id, fnode_pos
      double precision ::  xi, eta, zeta 
      double precision, parameter :: zero = 0.d0, one = 1.d0,
     &                               half = 0.5d0
c
c             tet faces nodes. have been ordered so corners
c             are listed first (enode)
c
c                face            corners        mid-edge
c                  1            1 - 3 - 2       7 - 6 - 5
c                  2            1 - 2 - 4       5 - 9 - 8
c                  3            2 - 3 - 4       6 - 10 - 9
c                  4            1 - 4 - 3       8 - 10 - 7
c
      select case ( fnode_pos )
      case( 1 )
       xi   = zero
       eta  = zero
       zeta = zero
...
c
     ...
     &              ' (tied_nodeiso).'
            else if(isotetpoint(i).gt.1.1d0)then
              mflag = 2
              message = 'internal error: tetrahedron isoparametric'//
     &              ' coordinate > 1.1; check the log file'//
     &              ' (tied_nodeiso).'
            end if
            if( mflag >= 2 ) then
