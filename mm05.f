...
c
     ...
     &     trace_eps_np1(mxvl), yf_vec(mxvl), mises_eqiv_eps_pls,
     &     ductile_damage, triax(mxvl)
      double precision, parameter :: zero = 0.0d0, one = 1.0d0,
     &     two = 2.0d0, half = 0.5d0, root23 = dsqrt(2.0d0/3.0d0) 
c
...
c
       ...
       call mm05_compute_ddczm( step, iter, span, iout, mxvl, felem, 
     &                          gpn, nstrs, hist_size, history_n, 
     &                          history_np1, stress_n, stress_np1, 
     &                          triax )
       do i = 1, span
        mises_eqiv_eps_pls = root23 * history_np1(i,3)
        ductile_damage     = history_np1(i,18)
        nonlocal_state(i,1) = -one
...
c    ****************************************************************
c    *                                                              *
c    *               subroutine mm05_compute_ddczm                  *
c    *                                                              *
...
c    ****************************************************************
c
      subroutine mm05_compute_ddczm( step, iter, span, iout, mxvl, 
     &                               felem, gpn, nstrs, hist_size, 
     &                               history_n, history_np1, 
     &                               stress_n, stress_n1, triax_n1 )
c
      use mod_damage_ddczm
      implicit none
c     
      integer, intent(in) :: 
     &    step, iter, span, iout, mxvl, felem, gpn, hist_size, nstrs
      double precision, intent(in) ::
     &    history_n(span,hist_size), stress_n(mxvl,nstrs), 
     &    stress_n1(mxvl,nstrs)
      double precision, intent(inout) :: history_np1(span,hist_size)
...
c 
     ...
     &       f_n, f_n1, f_0,
     &       vgi_n, vgi_n1, triax_mod
c
      integer, parameter :: hoffset = 14 ! uses (hoffset+1:hoffset+5)
      double precision, parameter :: 
     &       zero = 0.0d0, root23 = sqrt(2.0d0/3.0d0) 
c 
c 
c     init triax. if damage off, return. 
c  
...
c
      local_debug = .false.
      if( (felem+i-1) .eq. 3736 .and. gpn .eq. 2) local_debug = .true.
c
      if( local_debug ) then
...
c      
        local_debug = .false.
c        if( (felem+i-1) .eq. 3735 .and. gpn .eq. 2) local_debug = .true.
c       
c       retrieve history data
        peeq_n         = history_n(i,hoffset+1)
        dmg_intgrnd_n  = history_n(i,hoffset+2)
        dmg_intgrl_n   = history_n(i,hoffset+3)
        peeq_comp_n    = history_n(i,hoffset+5)
c       
c       retrieve current PEEQ
        peeq_n1  = root23 * history_np1(i,3)
c
        if( type_swdm ) then 
c 
...
c       
        triax_n1(i) = triax
        history_np1(i,hoffset+1) = peeq_n1
        history_np1(i,hoffset+2) = dmg_intgrnd_n1
        history_np1(i,hoffset+3) = dmg_intgrl_n1
        history_np1(i,hoffset+4) = damage ! *** ensure damage state index is same *** 
        history_np1(i,hoffset+5) = peeq_comp_n1
c 
        if( local_debug ) then 
          write(iout,9020)
          write(iout,9025) i, felem+i-1, mises, triax, lodeang, 
     &                     peeq_n1, peeq_comp_n1, dmg_intgrnd_n1, 
     &                     dmg_intgrl_n1, damage
        end if
c 
      end do ! over span
c
      return
c
 9000 format(/3x,"...... entered mm05_compute_ddczm .....",
     & /,10x,"step, iter, span, felem: ",i6,i3,i4,i7,
     & /,10x,"gpn, mxvl: ",i2,i4)
 9010 format(/7x,"mod_damage_ddczm values:"
     &       /9x,"vgi_crit",2x,"c_prop",2x,"kappa",2x,"lambda",
     &        2x,"ddczm_dmg_type")
 9015 format(11x,f6.3,2x,f6.3,2x,f6.3,2x,f6.3,7x,i3)
 9020 format(/7x,"damage calculations:"
     &       /9x,"i  ",2x,"elem",8x,"mises",3x,"triax",
     &        2x,"lode_ang",2x,"peeq",2x,"peeq_c",4x,"dmg_intgrnd",
     &        5x,"dmg_intgrl",6x,"damage")
 9025 format(7x,i3,1x,i7,6x,f8.3,1x,f7.4,2x,f6.3,3x,f5.3,
     &       2x,f5.3,4x,e11.4,3x,e11.4,3x,e11.4)
 9900 format(/9x,">>> Error detecting damage model. Terminating")
 9910 format(/9x,">>> ductile damage ddczm is not enabled!")
c
      end subroutine mm05_compute_ddczm
c *******************************************************************
c *                                                                 *
...
c
       do i = 1, span 
          ...
          history_np1(i,13)   = zero
          history_np1(i,14)   = zero
          history_np1(i,15)   = zero
          history_np1(i,16)   = zero
          history_np1(i,17)   = zero
          history_np1(i,18)   = zero
          history_np1(i,19)   = zero
       end do
c
c
...
c
     ...
     &                  Q_bar*(one-exp( -b*aps_np1 )))
         ...
         trial_stress(i,1:6) = sig_t(1:6)
          history_np1(i,13)   = 0.0d00
          history_np1(i,14)   = 0.0d00
          history_np1(i,15)   = 0.0d00
          history_np1(i,16)   = 0.0d00
          history_np1(i,17)   = 0.0d00
          history_np1(i,18)   = 0.0d00
          history_np1(i,19)   = 0.0d00
...
c *                           last modified 5/22/19 by vsp          *
c *                                                                 * 
c *                                                                 *
c *******************************************************************
c
...
c
              if( cut_step_size_now .and. (nsubinc.ge.3) ) return 
c 
...
c
      double precision, parameter :: 
     &    zero = 0.0d0, one = 1.0d0, two = 2.0d0, tol = 0.00001d0
c 
      debug = .false.
c 
      eta = zero
...
c
           if( c .eq. zero ) then
             if( a .gt. zero ) then
                eta = zero
             elseif( (a.eq.zero).and.(b.eq.zero) ) then 
c               devdeps==0, load reversal. a/b causes overflow. 
                eta = 1.0d0
             else 
                eta = -two*a/b
             end if
...
c 
c           if( eta .gt. one ) eta = zero ! VSP change 05/22/2019
c
...
c
       do i= 1, span
         ...
         elestr(i,11) = history(i,18)
c         elestr(i,11) = three
c         dword = history(i,4) 
c         if (iword(i) .eq. 1 ) then
c           elestr(i,11) = one
c         end if
       end do
c     
